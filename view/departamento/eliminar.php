...
<body class="p-3 mb-2 bg-info">
	...
	foreach ($departamento as $depto) {
		...
		<div class="container">
			<div class="jumbotron">
				<label class="display-4">Eliminar Departamento</label>
			</div>
			<form action="../../controller/departamento/eliminar.php" method="POST">
				<div class="form-row">
					<div class="form-group col-md-6">
						<h4>DEPARTAMENTO</h4>
						<br>
						<input type="hidden" name="dep_codigo" value="<?php echo $depto['dep_codigo'];?>">
						<input class="form-control" type="text" disabled name="dep_nombre" maxlength="30" value="<?php echo $depto['dep_nombre'];?>">
					</div>
				</div>
				<button type="submit" name="eliminar" class="btn btn-secondary">Eliminar</button>
			</form>
		</div>
	<?php } ?>
</body>
